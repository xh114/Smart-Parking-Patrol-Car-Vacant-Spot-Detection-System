...
#define PROTOCOL_MIN_LEN                            7u

#define CRC_CALC_START_POS                          2
#define CRC_CALC_LEN(payload_len)                   ((payload_len) + 3)
...
#define LOCATION_ID                                 (unsigned char)0x68
#define SPEED_ID                                    (unsigned char)0x70
#define STATUS_ID                                   (unsigned char)0x80
...
#define LOCATION_DATA_LEN                           (unsigned char)20
#define SPEED_DATA_LEN                              (unsigned char)12
#define STATUS_DATA_LEN                             (unsigned char)1
...
int yis_std_out_decoder::data_proc(unsigned char *data, unsigned int len, yis_out_data_t *result)
{
    ...
    // [ADDED] Decoder stays focused on raw telemetry; software yaw zero is applied later in the ROS node.
    i += PAYLOAD_POS;
    cnt = header->len;
    while (cnt)
    {
